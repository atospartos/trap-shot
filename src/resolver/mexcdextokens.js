...
async function hasDexLiquidity(tokenSymbol) {
    ...
    try {
        ...
        for (const pair of pairs) {
            ...
            if (liquidity > 10000 || volume24h > 1000) {
                return true;
            }
        }
        return false;
...
// ---------- 7. Получение цены с MEXC ----------
async function getMexcPrice(tokenSymbol) {
    try {
        const response = await axios.get(
            `https://api.mexc.com/api/v3/ticker/price?symbol=${tokenSymbol}USDT`,
            { timeout: 5000 }
        );
...
// ---------- 8. Получение цены с DEX ----------
async function getDexPrice(tokenSymbol) {
    const url = `https://api.dexscreener.com/latest/dex/search?q=${tokenSymbol}`;
...
async function findUniqueMexcTokens() {
    ...
    for (let i = 0; i < tokenList.length; i++) {
        ...
        const majorExchanges = [onBinance, onOKX, onBybit];
        ...
        const price = await getMexcPrice(token);
        const dexPrice = await getDexPrice(token);
        
        let deviation = null;
        if (price && dexPrice && dexPrice > 0) {
            deviation = ((price - dexPrice) / dexPrice) * 100;
        }
        
        // Категория A: чистые (нет ни на одной крупной бирже)
        if (majorCount === 0 && !onGate) {
            pureResults.push({
                symbol: token,
                category: 'PURE',
                price: price,
            ...
            console.log(`✅ [PURE] ${token} | Откл: ${deviation ? deviation.toFixed(2) + '%' : 'N/A'}`);
        }
        // Категория B: ровно одна крупная биржа
        else if (majorCount === 1 && !onGate) {
            let whichExchange = '';
            if (onBinance) whichExchange = 'Binance';
            else if (onOKX) whichExchange = 'OKX';
            else if (onBybit) whichExchange = 'Bybit';
            
            singleExchangeResults.push({
                symbol: token,
                category: 'SINGLE_EXCHANGE',
                presentOn: whichExchange,
                price: price,
                dexPrice: dexPrice,
                deviationPercent: deviation,
                timestamp: new Date().toISOString()
            });
            console.log(`🟡 [${whichExchange}] ${token} | Откл: ${deviation ? deviation.toFixed(2) + '%' : 'N/A'}`);
        }
    ...
    console.log(`   🟡 SINGLE (MEXC + DEX + 1 биржа): ${singleExchangeResults.length}`);
    
    return { pure: pureResults, singleExchange: singleExchangeResults };
}
...
async function main() {
    ...
    const { pure, singleExchange } = await findUniqueMexcTokens();
    const elapsedTime = ((Date.now() - startTime) / 1000).toFixed(1);
    
    // Сохраняем
    fs.writeFileSync( path.join(process.cwd(), 'data/tokens/pure_tokens.json'), JSON.stringify(pure, null, 2));
    fs.writeFileSync(path.join(process.cwd(), 'data/tokens/single_exchange_tokens.json'), JSON.stringify(singleExchange, null, 2));
    
    console.log(`\n💾 Результаты сохранены:`);
    console.log(`   pure_tokens.json — ${pure.length} токенов`);
    console.log(`   single_exchange_tokens.json — ${singleExchange.length} токенов`);
