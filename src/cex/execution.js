...
class ExecutionManager {
    ...
    async openPosition(symbol, direction, entryPrice, takeProfit, stopLoss, size) {
        ...
        if (order && order.id) {
            ...
            await this.placeOrder(symbol, side === 'buy' ? 'sell' : 'buy', takeProfit, size);
            await this.placeOrder(symbol, side === 'buy' ? 'sell' : 'buy', stopLoss, size);
        }
    }
    ...
    setupListeners() {
        eventEmitter.on('signal:open', (signal) => {
            this.openPosition(
                ...
                signal.entryCexPrice, signal.takeProfit, signal.stopLoss,
                signal.size || 100
            );
        });
